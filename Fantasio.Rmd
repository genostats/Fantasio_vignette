...
We are going to work on the Bedouin population, so after updating the stats in `x` (this adds several informations
such as allele frequencies, etc), we select this population:
...
```
In the case of "Hotspots", by default, we do not require to have a minimum number of markers in each segment (`minMarkers = 0`). 
